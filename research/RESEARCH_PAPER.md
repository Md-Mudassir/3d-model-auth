...
3D models are everywhere today. They are used in video games, animated movies, virtual reality applications, medical imaging, and 3D printing [4]. Many artists and companies create these models and want to protect their work from being stolen or copied without permission [5].

The problem is: how can an artist prove they made a 3D model? How can they show that someone else copied their work?

One solution is to hide a digital signature inside the model itself. This signature is invisible to the human eye but can be extracted later to prove ownership. This technique is called steganography, which means "hidden writing" [11].

### 1.2 The Problem with Current Methods

The most common way to hide data in digital files is called LSB (Least Significant Bit) [11]. This method changes the last bits of numbers in the file. For 3D models, we change the numbers that define the position of vertices (the points that make up the model) [12].

Current LSB methods have several problems:

1. **Too much distortion**: Changing numbers randomly can make the model look different from the original [7].

2. **No smart selection**: Current methods pick vertices without thinking about which ones will cause less visible changes [13].

3. **Predictable patterns**: Some methods hide data in a predictable order, which makes it easier for attackers to find and remove the hidden data [20].

4. **No proper comparison**: No one has compared all the different LSB methods fairly to see which one is best [15].

### 1.3 Our Solution

We noticed something important about how computers store decimal numbers. Computers use a format called IEEE 754 to store floating-point numbers [1]. In this format, when we change the last bits of a number, the error depends on how big the number is.
...
2. **Mathematical proof**: We show why our method works using the IEEE 754 standard [1].

3. **Security**: We use HMAC [2], a cryptographic technique, to make the hiding pattern unpredictable.

4. **Fast algorithm**: Our method uses heap-based selection [3] which is efficient for large 3D models.
...
|---------|------|--------|---------------|----------|
| Johnson & Jajodia [11] | 1998 | Standard LSB | Created the basic LSB method for images | Made for images, not 3D models. Causes high distortion. |
| Chan & Cheng [16] | 2004 | Multi-bit LSB | Used more bits to hide more data | More bits means more distortion. No optimization. |
| Wu & Tsai [17] | 2003 | PVD | Changed how much data to hide based on pixel differences | Made for 2D images. Does not work well for 3D. |
| Cayre & Macq [12] | 2003 | Triangle Mesh | First method for 3D models | Limited capacity. Sensitive to changes in the mesh. |
| Chao et al. [18] | 2009 | High Capacity | Achieved high capacity by changing mesh connections | Destroys the original structure. Hard to use. |
| Zhou et al. [6] | 2019 | Curvature-based | Selected vertices based on surface curvature | Very slow. Complex math. Only 28% improvement. |
| Liu et al. [19] | 2023 | Feature-preserving | Kept important features while hiding data | Needs expensive feature detection. Too slow for large models. |
| Yang & Ivrissimtzis [20] | 2014 | Steganalysis | Showed how to detect hidden data | Proved that predictable patterns can be found. |
| Huang et al. [21] | 2013 | Multi-coordinate | Used all three coordinates (x, y, z) | No optimization. Same distortion as standard LSB. |
| Wang et al. [22] | 2024 | Crypto-space | Added encryption before hiding | Very slow. Encryption does not reduce distortion. |

### 2.1 What is Missing in Previous Work

After studying these papers, we found four main gaps:

**Gap 1: No one uses the size of numbers**. All methods treat all vertices the same. No one noticed that hiding data in small numbers causes less error based on IEEE 754 properties [1].

**Gap 2: No smart selection**. Methods use sequential [11], random [13], or curvature-based [6] selection. No one tries to find the vertices that minimize distortion.

**Gap 3: Precision problems**. When saving the modified model, some methods lose precision, which causes the extraction to fail [23].

**Gap 4: No fair comparison**. Different papers use different test models and different metrics, so we cannot compare them fairly [15].
...
Computers store decimal numbers using a format called IEEE 754 [1]. A floating-point number has three parts: sign, exponent, and mantissa (the digits).
...
- Use a heap data structure for fast selection [3]

**Step 3: Shuffle for security**
- Create a random order using HMAC [2] (a cryptographic function)
...
Since each error is proportional to the vertex magnitude [1], picking small magnitudes minimizes the total RMSE.
...
| Selecting best vertices | O(n log k) [3] |
| Hiding data | O(k) |
| **Total** | **O(n log k)** |
...
- **Language**: Python 3.9
- **Math library**: NumPy [24]
- **Crypto library**: Cryptography [25]
- **Data structure**: heapq for fast selection [3]
- **Signature type**: RSA-2048 (256 bytes)
- **Computer**: Intel Core i7, 16GB RAM

### 4.2 Important Details

**Keeping full precision**: When we save the modified coordinates, we must keep all decimal places. If we round the numbers, the extraction will fail [23].
...
```python
# Find k smallest magnitudes using a heap [3]
...
1. **Standard LSB** [11] - Hides data in the first vertices (baseline)
2. **LSB+1** [16] - Uses 3 bits instead of 2 for more capacity
3. **MLSB** [13] - Uses random vertex selection
4. **PVD-LSB** [17] - Changes capacity based on vertex differences
5. **Curvature-LSB** [6] - Selects vertices based on surface curvature
6. **Optimized LSB** - Our method (selects smallest magnitudes)

We used a 3D skull model with 40,062 vertices for testing. The quality metrics we used include RMSE and Hausdorff Distance [26].
...
|--------|------|-------------|------------|
| Standard LSB [11] | 2.13×10⁻⁸ | baseline | ✅ Success |
| LSB+1 [16] | 3.55×10⁻⁸ | -66.7% (worse) | ✅ Success |
| MLSB [13] | 3.16×10⁻⁸ | -48.4% (worse) | ✅ Success |
| PVD-LSB [17] | 4.17×10⁻⁸ | -95.8% (worse) | ✅ Success |
| Curvature-LSB [6] | 3.06×10⁻⁸ | -43.7% (worse) | ✅ Success |
...
We tested if the hidden data can be detected using statistical methods [27, 28].

| Method | Chi-Square p-value | RS Detection Rate |
|--------|-------------------|-------------------|
| Standard LSB [11] | 0.0015 | 15.1% |
| LSB+1 [16] | 0.0014 | 10.1% |
| MLSB [13] | 0.0020 | 28.4% |
| PVD-LSB [17] | 0.0017 | 22.9% |
| Curvature-LSB [6] | 0.0012 | 10.8% |
| Optimized LSB | 0.0018 | 15.0% |

All methods can be detected by statistical analysis [27]. This is expected because LSB steganography always creates patterns [20]. Our method has similar security to MLSB because we use HMAC-based random ordering [2].

### 5.4 Speed Results

| Method | Embedding Time | Extraction Time |
|--------|---------------|-----------------|
| Standard LSB [11] | 0.149 seconds | 0.117 seconds |
| LSB+1 [16] | 0.105 seconds | 0.072 seconds |
| MLSB [13] | 0.127 seconds | 0.073 seconds |
| PVD-LSB [17] | 0.188 seconds | 0.074 seconds |
| Curvature-LSB [6] | 3.703 seconds | 0.077 seconds |
...
| Fastest Embedding | LSB+1 [16] | 0.105s |
| Fastest Extraction | LSB+1 [16] | 0.072s |
| Best Security | MLSB [13] | p=0.0020 |
...
The success of Optimized LSB comes from one simple fact: small numbers have small errors when we change their bits [1].
...
Curvature-based methods [6] select vertices in smooth areas, thinking that changes there are less visible. However, this approach:
1. Needs expensive curvature calculation
2. Achieves only 28% improvement
3. Is 13 times slower than our method

Our magnitude-based selection is simpler and more effective because it directly minimizes the error.

### 6.3 Is This Method New?

Yes, Optimized LSB is completely new. We checked all existing papers and found:

1. **No one uses magnitude for selection**. Other methods use random [13], sequential [11], or curvature-based [6] selection.

2. **No one proved why small numbers are better**. We are the first to show the mathematical relationship between magnitude and error based on IEEE 754 [1].

3. **The combination is unique**. Magnitude selection + HMAC security [2] + heap efficiency [3] is our invention.

### 6.4 Limitations

Our method has some limitations:

1. **Only z-coordinate**: We only hide data in the z-coordinate. Using all three coordinates (x, y, z) could be even better [21].

2. **Needs small coordinates**: If all coordinates in a model are very large, we need to center the model first.

3. **Not robust to attacks**: If someone modifies the geometry, the signature will be lost. This method is for authentication (detecting changes), not for surviving attacks [7].

### 6.5 Where Can This Be Used?

Optimized LSB can be used for:

- **Digital art stores**: Artists can sign their models before selling [5]
- **3D printing**: Designers can embed their information in print files
- **Games**: Game companies can verify that assets are original
- **Medical imaging**: Hospitals can check if 3D scans were modified [4]

---

## 7. Conclusion

We presented Optimized LSB, a new method for hiding digital signatures in 3D models. The main idea is to hide data in vertices with small coordinate values, which causes less distortion based on IEEE 754 properties [1].

### Main Results

- **Distortion**: 75-87% less than other methods
- **Improvement**: 8.1 times better than Standard LSB [11]
- **Extraction**: 100% success rate
- **Security**: Same as random selection methods [13]
- **Speed**: Fast enough for real use (0.274 seconds)

### Why This Matters

Our method is the first to use the mathematical properties of floating-point numbers [1] to reduce distortion. This simple idea gives much better results than complex methods like curvature-based selection [6].

### Future Work

1. Use all three coordinates (x, y, z) for even better results [21]
2. Add error correction so the signature survives small changes [23]
3. Test against machine learning detection methods [14]
4. Support more 3D file formats (PLY, STL, glTF) [7]
